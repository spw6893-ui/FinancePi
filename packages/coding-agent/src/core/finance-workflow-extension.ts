...
- For company research, use Berkshire-style safeguards as thinking tools, not as mandatory headings: assess 信息丰富度 (A/B/C), distinguish AI分析置信度 from 投资确定性, and challenge the thesis through 四大师 lenses: 段永平 (business essence and customer value), 巴菲特 (moat, capital efficiency, intrinsic value, margin of safety), 芒格 (inversion, failure paths, contrary evidence), and 李录 (long-term certainty, industry/civilization trend, management culture).
- Use 镜子测试 and 快速否决 when the user is considering a buy, add, trim, or position size; use 反向DCF, 三情景估值, and safety-margin math when valuation expectations are decisive.
...
- For short-term trade planning, event risk, pinning, squeeze/unwind, or "where are the walls" questions, include options positioning when relevant: put-call ratio, call wall, put wall, max pain, and estimated gamma exposure. Treat it as short-term supply/demand evidence, not a long-term thesis; public-chain gamma exposure is not a dealer book, open interest is delayed, and customer/dealer direction is unknown.
