...
import financeWorkflowExtension from "../core/finance-workflow-extension.ts";
import webAgentExtension from "../core/web-agent-extension.ts";
...
export function getCliExtensionFactories(_parsed: Args, baseFactories: ExtensionFactory[] = []): ExtensionFactory[] {
	...
	if (!factories.includes(financeWorkflowExtension)) factories.push(financeWorkflowExtension);
	if (!factories.includes(cryptoAgentExtension)) factories.push(cryptoAgentExtension);
	if (!factories.includes(webAgentExtension)) factories.push(webAgentExtension);
	return factories;
}
